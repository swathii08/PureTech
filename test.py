    ...
    if website_name:
        try:
            ...
            if title and price:
                ...
                GOOGLE_API_KEY = "AIzaSyBOtkTpO9dSa5hnwikzTlkATTu4imJlPCg"
                ...
                if st.button('Analyse Your Product'):
                    with st.expander("Ingredients"):
                        ...
                        gemini_final_response = st.session_state.chat_session.send_message(f" {merged_ingredients}. Please analyze and provide the ingredient list and put a ✅ emoji next to safe ingredients and a ❌ next to harmful ingredients which is not better for human health and Body. Don't include allergen information or extra details, stick to the ingredients only. and avoid same entries of ingridents")
                        safety_score_response = st.session_state.chat_session.send_message(f"Compute the number of unhealthy ingredients which is not good for human health in take {gemini_final_response} for analysis and compute the safety_score = 100 - 4 * number of unhealthy ingredients")
                    ...
                    category_recommendation = st.session_state.chat_session.send_message(f"Please suggest a 2 product in the same category as that of {title} that is better than the current product along with an {url} link keep the content precise and short to the point do not brief.")

                    reviews_summary = st.session_state.chat_session.send_message(f"Please analyze the top 4-5 customer reviews of {title} with given url {url} and do sentimental analysis. provide an overall summary keep it super short not exceeding more than four lines.")
